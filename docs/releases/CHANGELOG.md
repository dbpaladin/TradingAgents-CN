...
#### 🩹 单股分析并行消息污染修复
- **正式分析图恢复串行**: 将单股分析使用的 `setup_graph()` 从分析师并行扇出恢复为串行执行，避免共享 `messages` 造成跨分析师 tool call 污染
- **消息清理改为双模式**: `create_msg_delete()` 新增 `parallel_safe` 参数，正式分析链路执行真实消息清理，回测轻量图继续使用并行安全模式
- **根因明确**: 修复近期并行化后触发的 `No tool output found for function call ...` 隐性错误，该错误此前在服务层会表现为通用分析失败或 `NoneType` 相关异常

#### 📚 文档与归档
- **新增会话归档**: 归档本轮单股分析排障、修复与验证过程
- **更新性能优化文档**: 补充“回测可并行、正式单股分析需串行”的最终结论
- **更新会话索引**: 同步纳入 `history_chat/SESSION_INDEX.md`

## 📝 最新更新 (2026-03-24)

#### 📉 A股回测稳定性与可观测性修复
- **回测运行时约束增强**: 回测链路默认收紧记忆、辩论和风控讨论轮次，并在长区间任务中主动切换轻量模式，降低 AI 分析超时导致的整段 `HOLD`
- **强制入场保护移除**: 删除“连续空仓后硬性买入”的兜底逻辑，确保没有交易机会时保持真实空仓表现
- **耗时展示自适应单位**: 回测明细中的分步耗时改为按 `ms / s / 分钟 / 小时` 自动换算，便于识别真实性能瓶颈

#### 💸 资金证据完整性判断修复
- **资金面摘要新增完整度字段**: 补充证据完整度分数、缺失项和数据质量说明，明确区分“证据偏空”与“数据缺口”
- **提示词约束增强**: 分析师、交易员和风控链路统一改为“缺失证据只降低置信度，不可单独推导为卖出”
- **报告解释边界更清晰**: 对北向、融资融券等缺口统一标注为“未命中/未更新/不适用”，避免误导性偏空措辞

#### 🔌 第三方 Tushare 接入规则补齐
- **数据源管理层支持 `.env` 覆盖**: 当 `TUSHARE_ENDPOINT` 为第三方地址且 `TUSHARE_TOKEN` 有效时，即使数据库中的 `tushare.enabled=false`，系统仍会按 `.env` 启用 Tushare
- **优先级链路同步修复**: 回测/分析降级顺序读取与 provider 层保持一致，避免第三方 Tushare 被数据库旧配置提前跳过
- **新增回归测试**: 覆盖“第三方 endpoint 可绕过 disabled，官方 endpoint 不可绕过”的边界场景

#### 📚 文档与归档
- **新增修复说明**: 补充回测资金证据、超时瓶颈和第三方 Tushare 接入的说明文档
- **新增会话归档**: 归档本轮排障、修复和提交过程，便于后续回溯
- **更新会话索引**: 同步纳入 `history_chat/SESSION_INDEX.md`
...
#### 📈 A股回测能力补齐
- **新增 Web 版 A 股回测入口**: 提供回测任务创建、状态查询、结果查询、删除任务等基础能力
- **A股交易规则接入**: 回测过程纳入 T+1、涨跌停、佣金、印花税、最低佣金等约束
- **成本与速度控制增强**: 支持分析师选择、研究深度、独立模型指定，以及按交易日间隔复用 AI 信号
- **回测体验逐步完善**: 历史任务、当前步骤、预估重算次数、任务中心展示等回测可观察性增强

#### 🔌 Tushare 端点可配置化
- **新增 `TUSHARE_ENDPOINT`**: 支持官方端点之外的兼容 Tushare 服务
- **默认行为明确**: 未配置时继续使用官方 `http://api.tushare.pro`
- **代理场景说明补齐**: 推荐在代理环境中同步维护 `NO_PROXY`

#### 💸 A股资金面分析师
- **新增资金面分析师**: 独立分析龙虎榜、机构/游资风格、北向和融资融券线索
- **新增资金面工具**: 构建 `fund_flow_report`，输出资金风格判断、行动偏向和结构化摘要
- **主链路接入**: 资金面报告已接入研究员、交易员、风险管理和反思链路

#### 🧩 A股分析师扩展联动
- **新增 A股分析师组合**: `fund_flow`、`theme_rotation`、`institutional_theme` 已同步接入分析主链路
- **前后端入口打通**: 单股分析、回测、报告详情、导出和进度展示均可识别新报告类型
- **别名扩展补齐**: `sentiment` 相关别名在 A 股场景下已覆盖更多交易语境分析师

#### 🌐 前后端同步
- **分析师选择项扩展**: Vue 前端、Streamlit 页面、回测入口均新增 `fund_flow`
- **报告展示与导出同步**: 详情页、分析结果页、导出链路支持 `fund_flow_report`
- **别名增强**: 前端 `sentiment` 别名会自动扩展到 `social + emotion + fund_flow + theme_rotation`

#### ✅ 验证与环境修正
- **新增测试**: 增加 `tests/test_a_share_fund_flow.py`
- **A股相关测试通过**: `fund_flow + sentiment + institutional_theme` 共 7 个测试通过
- **开发环境约定明确**: 后续测试统一使用项目 `.venv`，避免误用系统 Python

### 📝 开发中更新 (2026-03-22)

#### 🛠️ 服务管理与启动链路整理
- **统一服务脚本**: 以 `app_services.sh` 和 `docker_services.sh` 替代旧的分散启动方式
- **开发热重载入口明确**: 支持通过 `BACKEND_RELOAD=1` 启用后端热重载
- **启动阻塞问题修复**: 后端启动期同步任务改为更稳健的后台处理方式，减少 reload 卡死

#### 🔐 认证与路由兼容修复
- **注册接口补齐**: 新增数据库认证链路中的注册接口，修复前端或脚本调用缺口
- **路由斜杠行为修正**: 调整 `redirect_slashes` 相关兼容处理，减少 `405` 与错误参数导致的启动问题
- **健康检查链路更清晰**: 前端与后端分别使用更合适的健康检查路径

#### 🌐 Docker / Nginx / API 路径修正
- **前端改用相对 API 路径**: 更适合经 Nginx 或统一入口部署，减少跨域和地址漂移问题
- **Nginx 代理链路补齐**: Docker 部署下支持更清晰的前后端代理路径
- **健康检查行为修复**: 修正容器编排与前端健康检查相关问题，提升启动成功率

#### 🧭 A股报告决策一致性修复
- **题材归类口径统一**: 题材轮动分析统一使用 `主线核心 / 主线外延 / 非主线`，避免将“主线相关但非核心”误写成“无主线关联”
- **机构布局评分解释增强**: 机构布局题材分析新增评分区间与依据要求，并在证据不足时明确标注数据缺口
- **最终建议分层输出**: 风险经理与交易员新增“持仓者建议 / 空仓者建议”，并要求显式解释子模块冲突如何裁决
- **缺失数据降权**: 北向、融资、机构席位等关键资金数据缺失时，不能再直接输出强结论
- **分析师别名补齐**: 前端 `sentiment` 别名现自动扩展到 `institutional_theme`

#### 🧩 A股报告口径第二阶段优化
- **题材标签源头修正**: 题材轮动工具层新增角色映射与角色说明，将主线中的跟风/补涨/边缘成员优先归为 `主线外延`
- **情绪与题材框架对齐**: A股情绪分析师新增“全市场退潮 vs 局部主线强化/抱团避险”并存解释要求
- **补涨情景补充**: 对非龙头个股强制补充板块内轮动/补涨可能的概率判断
- **辩论去复读**: 激进、保守、中性三位分析师新增不同证据点与反驳不重复约束，降低模板化复读

#### 💭 情绪面分析师量化增强（A股/港股）
- **从模板化输出升级为量化输出**：基于 `stock_news` 近7天样本计算情绪指数、热度指数、置信度、情绪动量和分布结构
- **降级策略完善**：MongoDB 不可用时自动降级到中文情绪摘要接口
- **美股分支稳健性提升**：改用 `get_reddit_company_news`，避免潜在函数名不一致问题

#### 📰 新闻分析师A股链路优化
- **中文源优先策略**：A股场景优先拉取中文财经新闻，样本充足时提前返回，减少无关外文源调用
- **统一工具A股兜底调整**：不再默认使用“OpenAI全球新闻”作为A股新闻兜底，改为数据库/AKShare/实时中文/Google中文链路
- **可追溯文档完善**：新增专项修复文档与会话归档文档

### 📝 开发中更新 (2026-03-24)

#### 📈 回测明细可观测性增强
- **模型来源可见**：回测交易明细新增 `ai_provider` / `ai_model` / `ai_runtime_mode`
- **耗时拆分可见**：新增 `analysis_elapsed_ms` / `decision_elapsed_ms` / `execution_elapsed_ms` / `day_elapsed_ms`
- **前端结果页增强**：新增“AI推理明细（按天）”，可直接查看模型与分步耗时

#### ⚙️ 回测速度周期策略调整
- **关闭自动改写**：不再根据区间长度和分析师权重自动把 `decision_interval_days` 改为 3/5
- **完全尊重用户设置**：回测执行周期以用户输入为准，避免“我没选却被改”的体验问题

#### 📡 实时行情采集降噪与回退增强
- **Tushare rt_k 无权限降噪**：已确认无权限时跳过重复调用，减少周期性错误噪音
- **单轮多源回退**：同一轮采集中连续尝试 Tushare / AKShare 东财 / AKShare 新浪，提高可用性
- **可预期网络抖动降级日志级别**：对临时网络错误按降级路径记录，减少误报故障感知

#### 🧾 股票代码输入体验与稳定性修复
- **分析页输入联想增强**：个股分析页输入股票代码后，新增“正在匹配股票名称”状态和名称提示标签
- **回测页输入联想增强**：A股回测页输入代码后支持防抖查询股票名称，并在进行中任务/历史任务中展示 `stock_name`
- **任务中心删除提示优化**：删除回测任务时，弹窗文案支持展示 `代码（名称）`
- **多市场搜索接口 500 修复**：修复 `/api/markets/{market}/stocks/search` 因 MongoDB `_id(ObjectId)` 引发的序列化异常
- **返回结果兜底清洗**：后端在查询阶段排除 `_id`，并在去重循环中追加 `_id` 清理，避免再次触发同类异常
