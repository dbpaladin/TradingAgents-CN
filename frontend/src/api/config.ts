...
export const configApi = {
  ...
  // 添加大模型厂家
  addLLMProvider(provider: Partial<LLMProvider>): Promise<ApiResponse<{ id: string }>> {
    return ApiClient.post('/api/config/llm/providers', provider)
  },

  // 更新大模型厂家
  updateLLMProvider(id: string, provider: Partial<LLMProvider>): Promise<ApiResponse<null>> {
    return ApiClient.put(`/api/config/llm/providers/${id}`, provider)
  },

  // 删除大模型厂家
  deleteLLMProvider(id: string): Promise<ApiResponse<null>> {
    return ApiClient.delete(`/api/config/llm/providers/${id}`)
  },

  // 启用/禁用大模型厂家
  toggleLLMProvider(id: string, isActive: boolean): Promise<ApiResponse<null>> {
    return ApiClient.patch(`/api/config/llm/providers/${id}/toggle`, { is_active: isActive })
  },

  // 迁移环境变量到厂家管理
  migrateEnvToProviders(): Promise<ApiResponse<any>> {
    return ApiClient.post('/api/config/llm/providers/migrate-env')
  },

  // 🆕 初始化聚合渠道厂家配置
  initAggregatorProviders(): Promise<ApiResponse<{ added_count: number; skipped_count: number }>> {
    return ApiClient.post('/api/config/llm/providers/init-aggregators')
  },

  // 测试厂家API
  testProviderAPI(providerId: string): Promise<ApiResponse<any>> {
  ...
  // 删除模型目录
  deleteModelCatalog(provider: string): Promise<ApiResponse<null>> {
    return ApiClient.delete(`/api/config/model-catalog/${provider}`)
  },

  // 初始化默认模型目录
  initModelCatalog(): Promise<ApiResponse<null>> {
  ...
  // 添加或更新大模型配置
  updateLLMConfig(config: Partial<LLMConfig>): Promise<ApiResponse<{ model_name: string }>> {
    return ApiClient.post('/api/config/llm', config)
  },

  // 删除大模型配置
  deleteLLMConfig(provider: string, modelName: string): Promise<ApiResponse<null>> {
    return ApiClient.delete(`/api/config/llm/${provider}/${modelName}`)
  },

  // 设置默认大模型
  setDefaultLLM(name: string): Promise<ApiResponse<{ default_llm: string }>> {
  ...
  // 添加数据源配置
  addDataSourceConfig(config: Partial<DataSourceConfig>): Promise<ApiResponse<{ name: string }>> {
    return ApiClient.post('/api/config/datasource', config)
  },

  // 设置默认数据源
  setDefaultDataSource(name: string): Promise<ApiResponse<{ default_data_source: string }>> {
    return ApiClient.post('/api/config/datasource/set-default', { name })
  },

  // 更新数据源配置
  updateDataSourceConfig(name: string, config: Partial<DataSourceConfig>): Promise<ApiResponse<null>> {
    return ApiClient.put(`/api/config/datasource/${name}`, config)
  },

  // 删除数据源配置
  deleteDataSourceConfig(name: string): Promise<ApiResponse<null>> {
  ...
  addMarketCategory(category: Partial<MarketCategory>): Promise<ApiResponse<{ id: string }>> {
    return ApiClient.post('/api/config/market-categories', category)
  },

  updateMarketCategory(id: string, category: Partial<MarketCategory>): Promise<ApiResponse<null>> {
    return ApiClient.put(`/api/config/market-categories/${id}`, category)
  },

  deleteMarketCategory(id: string): Promise<ApiResponse<null>> {
  ...
  addDataSourceToCategory(dataSourceName: string, categoryId: string, priority?: number): Promise<ApiResponse<null>> {
  ...
  removeDataSourceFromCategory(dataSourceName: string, categoryId: string): Promise<ApiResponse<null>> {
    return ApiClient.delete(`/api/config/datasource-groupings/${dataSourceName}/${categoryId}`)
  },

  updateDataSourceGrouping(dataSourceName: string, categoryId: string, updates: Partial<DataSourceGrouping>): Promise<ApiResponse<null>> {
    return ApiClient.put(`/api/config/datasource-groupings/${dataSourceName}/${categoryId}`, updates)
  },

  // 批量更新分类内数据源排序
  updateCategoryDataSourceOrder(categoryId: string, orderedDataSources: Array<{name: string, priority: number}>): Promise<ApiResponse<null>> {
  ...
  getSystemSettingsMeta(): Promise<{ items: SettingMeta[] }> {
    return ApiClient.get<{ items: SettingMeta[] }>('/api/config/settings/meta').then((r) => r.data)
  },
  ...
  // 添加数据库配置
  addDatabaseConfig(config: Partial<DatabaseConfig>): Promise<ApiResponse<null>> {
    return ApiClient.post('/api/config/database', config)
  },

  // 更新数据库配置
  updateDatabaseConfig(dbName: string, config: Partial<DatabaseConfig>): Promise<ApiResponse<null>> {
    return ApiClient.put(`/api/config/database/${encodeURIComponent(dbName)}`, config)
  },

  // 删除数据库配置
  deleteDatabaseConfig(dbName: string): Promise<ApiResponse<null>> {
  ...
  // 获取系统设置
  getSystemSettings(): Promise<ApiResponse<Record<string, any>>> {
    return ApiClient.get('/api/config/settings')
  },

  // 获取默认模型配置
  getDefaultModels(): Promise<{ quick_analysis_model: string; deep_analysis_model: string }> {
    return ApiClient.get<Record<string, any>>('/api/config/settings').then((response) => {
  ...
  // 更新系统设置
  updateSystemSettings(settings: Record<string, any>): Promise<ApiResponse<null>> {
  ...
  // 导出配置
  exportConfig(): Promise<ApiResponse<{ data: any; exported_at: string }>> {
    return ApiClient.post('/api/config/export')
  },

  // 导入配置
  importConfig(configData: Record<string, any>): Promise<ApiResponse<null>> {
    return ApiClient.post('/api/config/import', configData)
  },

  // 迁移传统配置
  migrateLegacyConfig(): Promise<ApiResponse<null>> {
    return ApiClient.post('/api/config/migrate-legacy')
  },

  // 配置重载
  reloadConfig(): Promise<ApiResponse<any>> {
