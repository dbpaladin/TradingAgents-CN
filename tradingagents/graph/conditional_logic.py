...
class ConditionalLogic:
    ...
    def should_continue_market(self, state: AgentState):
        ...
        logger.debug(f"🔀 [条件判断] should_continue_market")
        logger.debug(f"🔀 [条件判断] - 消息数量: {len(messages)}")
        logger.debug(f"🔀 [条件判断] - 报告长度: {len(market_report)}")
        logger.debug(f"🔧 [死循环修复] - 工具调用次数: {tool_call_count}/{max_tool_calls}")
        logger.debug(f"🔀 [条件判断] - 最后消息类型: {type(last_message).__name__}")
        logger.debug(f"🔀 [条件判断] - 是否有tool_calls: {hasattr(last_message, 'tool_calls')}")
        if hasattr(last_message, 'tool_calls'):
            logger.debug(f"🔀 [条件判断] - tool_calls数量: {len(last_message.tool_calls) if last_message.tool_calls else 0}")
            if last_message.tool_calls:
                for i, tc in enumerate(last_message.tool_calls):
                    logger.debug(f"🔀 [条件判断] - tool_call[{i}]: {tc.get('name', 'unknown')}")
        ...
        if market_report and len(market_report) > 100:
            logger.debug(f"🔀 [条件判断] ✅ 报告已完成，返回: Msg Clear Market")
            return "Msg Clear Market"

        # 只有AIMessage才有tool_calls属性
        if hasattr(last_message, 'tool_calls') and last_message.tool_calls:
            logger.debug(f"🔀 [条件判断] 🔧 检测到tool_calls，返回: tools_market")
            return "tools_market"

        logger.debug(f"🔀 [条件判断] ✅ 无tool_calls，返回: Msg Clear Market")
        return "Msg Clear Market"
    ...
    def should_continue_social(self, state: AgentState):
        ...
        logger.debug(f"🔀 [条件判断] should_continue_social")
        logger.debug(f"🔀 [条件判断] - 消息数量: {len(messages)}")
        logger.debug(f"🔀 [条件判断] - 报告长度: {len(sentiment_report)}")
        logger.debug(f"🔧 [死循环修复] - 工具调用次数: {tool_call_count}/{max_tool_calls}")
        ...
        if sentiment_report and len(sentiment_report) > 100:
            logger.debug(f"🔀 [条件判断] ✅ 报告已完成，返回: Msg Clear Social")
            return "Msg Clear Social"

        # 只有AIMessage才有tool_calls属性
        if hasattr(last_message, 'tool_calls') and last_message.tool_calls:
            logger.debug(f"🔀 [条件判断] 🔧 检测到tool_calls，返回: tools_social")
            return "tools_social"

        logger.debug(f"🔀 [条件判断] ✅ 无tool_calls，返回: Msg Clear Social")
        return "Msg Clear Social"
    ...
    def should_continue_emotion(self, state: AgentState):
        ...
        logger.debug(f"🔀 [条件判断] should_continue_emotion")
        logger.debug(f"🔀 [条件判断] - 消息数量: {len(messages)}")
        logger.debug(f"🔀 [条件判断] - 报告长度: {len(report)}")
        logger.debug(f"🔧 [死循环修复] - 工具调用次数: {tool_call_count}/{max_tool_calls}")
        ...
        if report and len(report) > 100:
            logger.debug("🔀 [条件判断] ✅ 报告已完成，返回: Msg Clear Emotion")
            return "Msg Clear Emotion"

        if hasattr(last_message, "tool_calls") and last_message.tool_calls:
            logger.debug("🔀 [条件判断] 🔧 检测到tool_calls，返回: tools_emotion")
            return "tools_emotion"

        logger.debug("🔀 [条件判断] ✅ 无tool_calls，返回: Msg Clear Emotion")
        return "Msg Clear Emotion"
    ...
    def should_continue_fund_flow(self, state: AgentState):
        ...
        logger.debug("🔀 [条件判断] should_continue_fund_flow")
        logger.debug(f"🔀 [条件判断] - 消息数量: {len(messages)}")
        logger.debug(f"🔀 [条件判断] - 报告长度: {len(report)}")
        logger.debug(f"🔧 [死循环修复] - 工具调用次数: {tool_call_count}/{max_tool_calls}")
        ...
        if report and len(report) > 100:
            logger.debug("🔀 [条件判断] ✅ 报告已完成，返回: Msg Clear Fund_flow")
            return "Msg Clear Fund_flow"

        if hasattr(last_message, "tool_calls") and last_message.tool_calls:
            logger.debug("🔀 [条件判断] 🔧 检测到tool_calls，返回: tools_fund_flow")
            return "tools_fund_flow"

        logger.debug("🔀 [条件判断] ✅ 无tool_calls，返回: Msg Clear Fund_flow")
        return "Msg Clear Fund_flow"
    ...
    def should_continue_theme_rotation(self, state: AgentState):
        ...
        logger.debug("🔀 [条件判断] should_continue_theme_rotation")
        logger.debug(f"🔀 [条件判断] - 消息数量: {len(messages)}")
        logger.debug(f"🔀 [条件判断] - 报告长度: {len(report)}")
        logger.debug(f"🔧 [死循环修复] - 工具调用次数: {tool_call_count}/{max_tool_calls}")
        ...
        if report and len(report) > 100:
            logger.debug("🔀 [条件判断] ✅ 报告已完成，返回: Msg Clear Theme_rotation")
            return "Msg Clear Theme_rotation"

        if hasattr(last_message, "tool_calls") and last_message.tool_calls:
            logger.debug("🔀 [条件判断] 🔧 检测到tool_calls，返回: tools_theme_rotation")
            return "tools_theme_rotation"

        logger.debug("🔀 [条件判断] ✅ 无tool_calls，返回: Msg Clear Theme_rotation")
        return "Msg Clear Theme_rotation"
    ...
    def should_continue_institutional_theme(self, state: AgentState):
        ...
        logger.debug("🔀 [条件判断] should_continue_institutional_theme")
        logger.debug(f"🔀 [条件判断] - 消息数量: {len(messages)}")
        logger.debug(f"🔀 [条件判断] - 报告长度: {len(report)}")
        logger.debug(f"🔧 [死循环修复] - 工具调用次数: {tool_call_count}/{max_tool_calls}")
        ...
        if report and len(report) > 100:
            logger.debug("🔀 [条件判断] ✅ 报告已完成，返回: Msg Clear Institutional_theme")
            return "Msg Clear Institutional_theme"

        if hasattr(last_message, "tool_calls") and last_message.tool_calls:
            logger.debug("🔀 [条件判断] 🔧 检测到tool_calls，返回: tools_institutional_theme")
            return "tools_institutional_theme"

        logger.debug("🔀 [条件判断] ✅ 无tool_calls，返回: Msg Clear Institutional_theme")
        return "Msg Clear Institutional_theme"
    ...
    def should_continue_news(self, state: AgentState):
        ...
        logger.debug(f"🔀 [条件判断] should_continue_news")
        logger.debug(f"🔀 [条件判断] - 消息数量: {len(messages)}")
        ...
        if news_report and len(news_report) > 100:
            logger.debug(f"🔀 [条件判断] ✅ 报告已完成，返回: Msg Clear News")
            return "Msg Clear News"

        # 只有AIMessage才有tool_calls属性
        if hasattr(last_message, 'tool_calls') and last_message.tool_calls:
            logger.debug(f"🔀 [条件判断] 🔧 检测到tool_calls，返回: tools_news")
            return "tools_news"

        logger.debug(f"🔀 [条件判断] ✅ 无tool_calls，返回: Msg Clear News")
        return "Msg Clear News"
    ...
    def should_continue_fundamentals(self, state: AgentState):
        ...
        logger.debug(f"🔀 [条件判断] should_continue_fundamentals")
        logger.debug(f"🔀 [条件判断] - 消息数量: {len(messages)}")
        logger.debug(f"🔀 [条件判断] - 报告长度: {len(fundamentals_report)}")
        logger.debug(f"🔧 [死循环修复] - 工具调用次数: {tool_call_count}/{max_tool_calls}")
        logger.debug(f"🔀 [条件判断] - 最后消息类型: {type(last_message).__name__}")
        
        # 🔍 [调试日志] 打印最后一条消息的详细内容
        logger.debug(f"🤖 [条件判断] 最后一条消息详细内容:")
        logger.debug(f"🤖 [条件判断] - 消息类型: {type(last_message).__name__}")
        if hasattr(last_message, 'content'):
            content_preview = last_message.content[:300] + "..." if len(last_message.content) > 300 else last_message.content
            logger.debug(f"🤖 [条件判断] - 内容预览: {content_preview}")
        ...
        if hasattr(last_message, 'tool_calls'):
            logger.debug(f"🔀 [条件判断] - tool_calls数量: {len(last_message.tool_calls) if last_message.tool_calls else 0}")
            if last_message.tool_calls:
                logger.debug(f"🔧 [条件判断] 检测到 {len(last_message.tool_calls)} 个工具调用:")
                for i, tc in enumerate(last_message.tool_calls):
                    logger.debug(f"🔧 [条件判断] - 工具调用 {i+1}: {tc.get('name', 'unknown')} (ID: {tc.get('id', 'unknown')})")
                    if 'args' in tc:
                        logger.debug(f"🔧 [条件判断] - 参数: {tc['args']}")
            else:
                logger.debug(f"🔧 [条件判断] tool_calls为空列表")
        else:
            logger.debug(f"🔧 [条件判断] 无tool_calls属性")

        # ✅ 优先级1: 如果已经有报告内容，说明分析已完成，不再循环
        if fundamentals_report and len(fundamentals_report) > 100:
            logger.debug(f"🔀 [条件判断] ✅ 报告已完成，返回: Msg Clear Fundamentals")
        ...
        if hasattr(last_message, 'tool_calls') and last_message.tool_calls:
            ...
            logger.debug(f"🔀 [条件判断] 🔧 检测到tool_calls，返回: tools_fundamentals")
            return "tools_fundamentals"

        # ✅ 优先级3: 没有tool_calls，正常结束
        logger.debug(f"🔀 [条件判断] ✅ 无tool_calls，返回: Msg Clear Fundamentals")
        return "Msg Clear Fundamentals"
    ...
    def should_continue_debate(self, state: AgentState) -> str:
        ...
        # 🔍 详细日志
        logger.debug(f"🔍 [投资辩论控制] 当前发言次数: {current_count}, 最大次数: {max_count} (配置轮次: {self.max_debate_rounds})")
        logger.debug(f"🔍 [投资辩论控制] 当前发言者: {current_speaker}")

        if current_count >= max_count:
            logger.debug(f"✅ [投资辩论控制] 达到最大次数，结束辩论 -> Research Manager")
            return "Research Manager"

        next_speaker = "Bear Researcher" if current_speaker.startswith("Bull") else "Bull Researcher"
        logger.debug(f"🔄 [投资辩论控制] 继续辩论 -> {next_speaker}")
        return next_speaker
    ...
    def should_continue_risk_analysis(self, state: AgentState) -> str:
        ...
        # 🔍 详细日志
        logger.debug(f"🔍 [风险讨论控制] 当前发言次数: {current_count}, 最大次数: {max_count} (配置轮次: {self.max_risk_discuss_rounds})")
        logger.debug(f"🔍 [风险讨论控制] 最后发言者: {latest_speaker}")

        if current_count >= max_count:
            logger.debug(f"✅ [风险讨论控制] 达到最大次数，结束讨论 -> Risk Judge")
            return "Risk Judge"
        ...
        logger.debug(f"🔄 [风险讨论控制] 继续讨论 -> {next_speaker}")
        return next_speaker
