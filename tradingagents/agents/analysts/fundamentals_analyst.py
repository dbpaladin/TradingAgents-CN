...
def create_fundamentals_analyst(llm, toolkit):
    ...
    def fundamentals_analyst_node(state):
        ...
        system_message = (
            ...
            "- 分析当前股价是否被低估或高估"
            "- 必须提供保守/基准/乐观三种估值情景，并明确每种情景的逻辑、价位区间和对应条件"
            "- 必须明确“基准情景目标价”作为后续交易模块的核心参考价"
            "- 包含PE、PB、PEG等估值指标分析"
            "- 结合市场特点进行分析"
            "🌍 语言和货币要求："
            "- 所有分析内容必须使用中文"
            ...
            if current_tool_calls > 0:
                ...
                if has_tool_result:
                    ...
                    force_system_prompt = (
                        ...
                        f"3. 当前股价是否被低估或高估的判断\n"
                        f"4. 保守/基准/乐观三种估值情景、各自价位区间和触发条件\n"
                        f"5. 明确的基准情景目标价\n"
                        f"6. 基于基本面的投资建议（买入/持有/卖出）\n\n"
                        f"要求：\n"
                        f"- 使用中文撰写报告\n"
                        f"- 基于消息历史中的真实数据进行分析\n"
                        f"- 分析要详细且专业\n"
                        f"- 投资建议必须明确（买入/持有/卖出）\n"
                        f"- 不允许只给单一静态目标价，必须写出三情景估值"
                    )
